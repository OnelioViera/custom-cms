...
export async function getContent(contentTypeId: string, status = "published") {
  try {
    const response = await fetchAPI(`/cms/${SITE_ID}/content/${contentTypeId}?status=${status}`)
...
export async function getContentById(contentTypeId: string, contentId: string) {
  try {
    const response = await fetchAPI(`/cms/${SITE_ID}/content/${contentTypeId}/${contentId}`)
...
export async function searchContent(contentTypeId: string, query: string) {
  try {
    const response = await fetchAPI(`/cms/${SITE_ID}/content/${contentTypeId}?q=${encodeURIComponent(query)}`)
    return response.data?.content || []
  } catch (error) {
