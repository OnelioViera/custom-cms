...
export default function Navigation({ currentPage, showTestimonials = false }: NavigationProps) {
    ...
    // Always render the nav structure, but swap content after mount
    return (
        <nav className="fixed w-full bg-white shadow-sm z-50 border-b border-gray-100" suppressHydrationWarning>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between" suppressHydrationWarning>
                {isClient ? (
                ...
                <div className="hidden md:flex gap-8" suppressHydrationWarning>
                    {isClient ? (
                        <>
                            <Link 
                                href="/projects" 
